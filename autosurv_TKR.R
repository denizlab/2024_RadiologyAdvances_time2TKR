...
#' @param trainData data frame containing covariates for prediction
#' @param testData data frame containing covariates for prediction
    ...
    if ("rsf" %in% trainModels){
        ...
        rf_cv <- function(nodesize, mtry){
            ...
            for (i in seq_len(cvFold)){
                ...
                importance_values <- varImp(rfsrcModel)
            ...
            list(Score = neg_brier, Pred = scoreCV, Importance = importance_values)
        }
        ...
        importance_rsf <- OPT_Res$Importance
