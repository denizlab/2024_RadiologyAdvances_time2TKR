...
##################################################################################
# Read the CSV file.
file_path <- "Data_Preparation/Datasets/SSL_TWIST_Selected_DESS_TSE_Xray_MOAKS_Quantitative_Radiographic_Imputed.csv"
...
source("D:/Survival_Analysis_Project/autoSurv-master/R/autosurv_TKR.R")
##################################################### ##################################################################################################### ################################################
TKRdata.results <- autosurv_TKR(timeVar = "survTime",
                                 statusVar = "statusComp",
                                 trainDat = TKRdata.dat_train,
                                 testDat = TKRdata.dat_validation,
                                 times =  c(1, 2, 3, 4, 5, 6, 7, 8, 9),
                                 trainModels = c("rsf"), 
                                 bins.upper = 36,
                                 testProp = 0.2/(1-0.2), # We dont use this since we provide train and test data separately
                                 seed = 1101,
                                 cv = FALSE,
                                 verbose.opt = FALSE)
###############################################################################################################################################
# TKRdata.results <- readRDS("models/RSF_models/SSL_TWIST_Selected_DESS_TSE_Xray_MOAKS_Quantitative_Radiographic_Imputed.RDS")
###############################################################################################################################################
###############################################################################################################################################
...
calculate_cindex <- function(predictions, true_times, event_status)
{
surv_obj <- Surv(true_times, event_status) # Create a Surv object with event status
c_index <- concordance(surv_obj ~ predictions)$concordance
return(round(c_index * 100, 1))
}
# Calculate Concordance Index (C-index) for the current model
cindex <- calculate_cindex(sum_of_1sTest, round(TKRdata.dat_test$survTime), TKRdata.dat_test$statusComp)
...
###############################################################################################################################################
# # To save the predicted years and predictions results(0 is wrong prediction 1 is true prediction) in a csv file
combined_correctly_predicted_data_yearsTest = data.frame(Labels = round(TKRdata.dat_test$survTime), PredictedYear = correctly_predicted_yearsTest, PredictionResult = correctly_predicted_dataTest)

file_save_dir <- "models/RSF_models/Results"

combined_file_path <- file.path(file_save_dir, file_name)

write.csv(combined_correctly_predicted_data_yearsTest, file = combined_file_path, row.names = FALSE)
########################################################################################################################

file_name_save <- basename(combined_file_path)

model_dir <- paste0("models/RSF_models/", gsub("\\.csv", "", file_name_save), ".RDS")

if (!file.exists(dirname(model_dir))) {
  dir.create(dirname(model_dir), recursive = TRUE)
}
saveRDS(TKRdata.results, file = model_dir)
################################################################################################################################################
